...
function generarPregunta() {
    ...
    const operadoresSeleccionados = Array.from({ length: numCartas - 1 }, () => 
        operacionesPosibles[Math.floor(Math.random() * operacionesPosibles.length)]
    );

    const valoresNumericos = cartasSeleccionadas.map(c => obtenerValorCarta(c));
    respuestaCorrecta = calcularResultadoCorrecto(valoresNumericos, operadoresSeleccionados);
